...
class Astar:  # Using BFS in this class too as it is derived of A* with g = 0
    ...
    def Astar(self, iteration, board, size, max_length, name, stringName):
        ...
        while self.open_list:
            ...
            self.output_parser.create_search_files(iteration, name, f, g, h,
                                                   current_node[1].get_current_board().transform_2d_to_1d())
            ...
            for i in range(len(possible_moves)):
                children_to_append = Board.Board(int(size),
                                                 current_node[1].get_current_board().prioritize_board(possible_moves)[
                                                     i])
                totalEstimate = self.heuristic(children_to_append.board)  # For BFS
                if name == "astar":
                    totalEstimate = current_node[1].get_depth() + 1 + self.heuristic(
                        children_to_append.board)  # heuristic for A*
            ...
            children_to_add = current_node[1].get_children()

            for child in children_to_add:

                if child not in self.open_list and child not in self.closed_list:  # If not in open/closed list
                    heapq.heappush(self.open_list, (child.get_estimate(), child))

                elif child in self.open_list and child not in self.closed_list:  # In the open_list, and not in the closed...

                    # Get index of first child 0 O(n)
                    index = self.open_list.index(child)
                    node_to_replace = self.open_list[index]
                    if node_to_replace.get_estimate() > child.get_estimate():
                        self.open_list.remove(node_to_replace)  # Removes le
                        heapq.heappush(self.open_list, (child.get_estimate(), child))

                elif child not in self.open_list and child in self.closed_list:  # In the closed list, and not in the open list

                    # Get index of first child 0 O(n)
                    index = self.closed_list.index(child)
                    node_to_replace = self.closed_list[index]
